...
public class ClientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clientManager.setOnClientListener(new WebSocketClientManager.OnClientListener() {
            ...
            public void onBinaryMessage(ByteBuffer data) {
                appendLog("← [二进制] " + data.remaining() + " 字节");
            }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        new Thread(() -> {
            try {
                ...
                clientManager.send(ByteBuffer.wrap(bytes));
                appendLog("→ [二进制] " + (name != null ? name : "未知文件") + " (" + len + " 字节)");
            } catch (Exception e) {
    ...
    private byte[] readBytesFromUri(Uri uri) {
        try (InputStream is = getContentResolver().openInputStream(uri);
            ...
            Log.w("ClientActivity", "读取文件失败: " + e.getMessage());
            return null;
        }
    }

...
适用于把本项目的 Android Library 模块发布到 JitPack，供外部通过 `implementation 'com.github.<github-user>:<repo>:<version>'` 引用。
...
> 以本仓库 `jingyao6/WebSocketScan` 的 `v1.0.7` 为例：
> ```bash
...
dependencies {
    implementation 'com.github.<github-user>:<repo>:<version>'
}
```
...
```bash
# 看所有 version 状态
curl -s "https://jitpack.io/api/builds/com.github.<github-user>/<repo>"

# 看某个 version 详细状态
curl -s "https://jitpack.io/api/builds/com.github.<github-user>/<repo>/<version>"

# 看 build log
curl -s -L "https://jitpack.io/com/github/<github-user>/<repo>/<version>/build.log"
```

> 以本仓库 `jingyao6/WebSocketScan` 的 `1.0.7` 为例：
> ```bash
